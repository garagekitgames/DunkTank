    ...
    public class GraphicalDemoSpawnerPooled : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (_counter > SpawnDelay)
            {
                ...
                var poolItem = _pool.SpawnPoolItem(transform.position + new Vector3(0, 0, Random.Range(-2, 2)), Quaternion.identity);
                poolItem.Rigidbody.velocity = transform.up*Velocity;

                _counter = 0;
                _totalTime += watch.ElapsedTicks;
            }

            Display.text = string.Format("Pooled Time: {0:0.000} ms", _totalTime / System.Diagnostics.Stopwatch.Frequency);
        }
