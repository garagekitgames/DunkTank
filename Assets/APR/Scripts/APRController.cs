...
public class APRController : MonoBehaviour
{
    ...
    public void LookAtTarget()
    {
        ...
        Debug.DrawRay(APR_Parts[0].transform.position, dirToLookTarget * 3, Color.green);

        //test.transform.eulerAngles = Vector3.up * targetAngle;

        //Debug.Log(targetAngle);


        APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation = Quaternion.Slerp(APR_Parts[0].GetComponent<ConfigurableJoint>().targetRotation, Quaternion.Inverse(Quaternion.Euler(Vector3.up * targetAngle) * startingRotation), Time.time * speed);
